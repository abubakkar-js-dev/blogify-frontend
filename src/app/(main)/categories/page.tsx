  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 md:pt-20 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
        ...
        >
          <span className="text-teal-600 font-bold text-sm tracking-widest uppercase mb-4 block">Categories</span>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 leading-tight">
            Explore your <br className="hidden sm:block" /> <span className="text-teal-500">passions.</span>
          </h1>
        </motion.div>
        ...
        >
          Browse through our curated collection of topics and find stories that resonate with you.
        </motion.p>
      </div>
        ...
        {categories.map((cat, idx) => (
          <Link key={cat.name} href={`/blogs?category=${cat.name}`}>
            <motion.div 
              ...
              className="group relative cursor-pointer rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden aspect-[4/3] shadow-lg shadow-slate-200/50"
            >
              <Image 
                src={cat.image} 
                alt={cat.name}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/20 to-transparent transition-opacity group-hover:opacity-90" />
              
              <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end">
                <div>
                  <div className={`w-10 h-10 ${cat.color} rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:-translate-y-1`}>
                    {cat.icon}
                  </div>
      ...
      <section className="mt-32">
        <div className="bg-white border border-slate-100 rounded-[3rem] p-8 md:p-16 flex flex-col lg:flex-row gap-12 items-center">
          <div className="flex-1">
            <h2 className="text-3xl font-bold text-slate-900 mb-6">Can't find what you're looking for?</h2>
            <p className="text-slate-500 mb-8 leading-relaxed">
              Our community is constantly growing. If you have a unique voice or a niche topic, start your own blog and build a community around it.
            </p>
          ...
          <div className="flex-1 w-full relative aspect-square lg:aspect-auto h-[300px] rounded-[2rem] overflow-hidden">
             <Image 
